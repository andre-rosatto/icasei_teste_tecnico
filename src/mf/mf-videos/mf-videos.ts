...
class MFVideos extends HTMLElement {
	static observedAttributes = ['searchbar', 'filter'];
	private root: ShadowRoot;
	private userData: string[];
	private searchBar: boolean | null = null;
	private filter: 'none' | 'favorites' = 'none';
	...
	attributeChangedCallback(name: string, _: string, newValue: string) {
		...
		this.createHTML();
		if (this.filter === 'favorites') this.getVideos();
	}
	...
	toggleFavorite(videoId: string, svg: SVGElement) {
	...
	getVideos() {
		const container = (this.root.querySelector('#mf-videos__search-results') as HTMLElement);
		container.innerHTML = '';
	...
	populateVideoList(videoIds: string[]) {
		const container = (this.root.querySelector('#mf-videos__search-results') as HTMLElement);
		container.innerHTML = '';
	...
	createHTML() {
		...
		if (this.searchBar) {
			(this.root.querySelector('#video-search') as HTMLInputElement).addEventListener('input', e => this.onVideoSearchInputChange(e));
			(this.root.querySelector('#mf-videos__search-icon') as SVGElement).addEventListener('click', () => this.onVideoSearchClick());
		}

		// DEBUG
		if (this.searchBar) this.onVideoSearchClick();
	}

	onVideoSearchInputChange(e: Event) {
		const target = e.target as HTMLInputElement;
		if (target.value.trim().length === 0) {
			(this.root.querySelector('#mf-videos__search-icon') as SVGElement).classList.add('disabled');
			target.classList.add('empty');
		} else {
			(this.root.querySelector('#mf-videos__search-icon') as SVGElement).classList.remove('disabled');
